...
Bagshui:AddComponent(function()
	...
	function Inventory:InitUi()
		...
		frames.searchBox.bagshuiData.searchIcon.bagshuiData.tooltipText = string.format(L.Toolbar_Search_TooltipText,
		...
		local function CreateSpaceSummary(parent, justifyH)
			...
			spaceSummary:SetHitRectInsets(-(BsSkin.bagBarSpacing / 2), -(BsSkin.bagBarSpacing / 2),
				-(BsSkin.bagBarSpacing / 2), -(BsSkin.bagBarSpacing / 2))
		...
		frames.spaceSummary:SetPoint("LEFT", buttons.bagSlots[table.getn(buttons.bagSlots)], "RIGHT",
			BsSkin.bagBarSpacing - 2, 0)
		...
		local miniTooltipName = tooltips.mini.bagshuiData
			.name -- We'll need to know the name in a bit to grab the TextLeft1 font string.
	...
	function Inventory:PopulateToolbarAndMainMenuItems()
		...
		self.toolbarAndMainMenuItems = {
			...
			{
				name = "StackStacks",
				texture = "StackStacks.tga",
				tooltipTitle = L.stackItems_TooltipTitle,
				tooltipText = L.stackItems_TooltipText,
				xOffset = -BsSkin.toolbarGroupSpacing,
				onClick = function()
					self.settings.stackItems = not self.settings.stackItems
				end,
				_bagshuiCheckedFunc = function()
					return self.settings.stackItems
				end,
			},
	...
	function Inventory:RescueWindow(requested)
		...
		if rescued then
			Bagshui:Print(requested and L.SettingReset_WindowPositionManual or L.SettingReset_WindowPositionAuto,
				self.inventoryTypeLocalized)
