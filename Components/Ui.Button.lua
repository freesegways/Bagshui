...
Bagshui:AddComponent(function()
	...
	function Ui:CreateIconButton(buttonOpts)
		...
		if buttonOpts.onClick then
			...
			button:SetScript("OnMouseDown", function()
				if not _G.this.bagshuiData then return end
				if not _G.this.bagshuiData.highlightTexture then
					_G.this.bagshuiData.highlightTexture = _G.this:GetHighlightTexture()
				end
				-- It's possible for there to be no highlight texture.
				if _G.this.bagshuiData.highlightTexture then
			...
			button:SetScript("OnMouseUp", function()
				if
					_G.this.bagshuiData
					and _G.this.bagshuiData.highlightTexture
				then
					_G.this.bagshuiData.highlightTexture:ClearAllPoints()
	...
	function Ui:SetIconButtonColors(button, vertexColor, hoverVertexColor)
		assert(type(button) == "table" and button.GetNormalTexture,
	...
	function Ui:UpdateIconButtonCooldown(button)
		...
		if button.bagshuiData.cooldownRemaining <= 0 then
			...
			button.bagshuiData.cooldownTexture:SetPoint("TOP", button, "BOTTOM", 0,
	...
	local function IconButton_ShineFrame_OnUpdate()
		...
		if iconButton_ShineFrame_OnUpdate_alpha < 0.1 then
			...
			_G.this.bagshuiData.shine:SetHeight(iconButton_ShineFrame_OnUpdate_alpha * _G.this:GetHeight() *
			ICON_BUTTON_SHINE_SCALE)
			_G.this.bagshuiData.shine:SetWidth(iconButton_ShineFrame_OnUpdate_alpha * _G.this:GetWidth() *
			ICON_BUTTON_SHINE_SCALE)
	...
	function Ui:ShineIconButton(button)
		...
		button.bagshuiData.shineFrame.bagshuiData.shine:SetHeight(button.bagshuiData.shineFrame:GetHeight() *
		ICON_BUTTON_SHINE_SCALE)
		button.bagshuiData.shineFrame.bagshuiData.shine:SetWidth(button.bagshuiData.shineFrame:GetWidth() *
		ICON_BUTTON_SHINE_SCALE)
	...
	function Ui:ShowIconButtonTooltip(button, tooltipDelayOverride, secondPass)
		...
		if bagshuiInfo.tooltipFunction then
			...
			then
				tooltip:AddLine(bagshuiInfo.tooltipText, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b,
					true)
			end
