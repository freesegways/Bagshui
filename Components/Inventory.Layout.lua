...
Bagshui:AddComponent(function()
	...
	function Inventory:Update(cascade)
		...
		if update_cascadeInventory then
			...
			update_cascadeInventory.windowUpdateNeeded = update_cascadeInventory.windowUpdateNeeded or
				self.windowUpdateNeeded
		end
	...
	function Inventory:UpdateWindow()
		...
		if self.windowUpdateNeeded then
			...
			local groupPadding = self.settings.groupPadding + (BsSkin.groupPaddingFudge or 0) +
			...
			local showGroupLabels = (self.settings.showGroupLabels and not self.settings.hideGroupLabelsOverride) or
				self.editMode
					...
					for columnNum = rowGroupStart, rowGroupEnd, rowGroupStep do
						...
						if self.groupWidthsInItems[groupId] > 0 then
							...
							if showGroupLabels then
								...
								if self.editMode then
									group.bagshuiData.labelFrame:SetWidth(self.actualGroupWidths[groupId] -
										(groupPadding * 2) - itemSlotMargin)
									group.bagshuiData.labelFrame:SetPoint("TOPLEFT", group, "TOPLEFT",
										BsSkin.groupLabelEditModeXOffset, BsSkin.groupLabelEditModeYOffset)
									group.bagshuiData.labelFrame:EnableMouse(false)
								else
									group.bagshuiData.labelFrame:SetWidth(self.actualGroupWidths[groupId] +
										BsSkin.groupLabelRelativeWidth)
									group.bagshuiData.labelFrame:SetPoint("BOTTOMLEFT", group, "TOPLEFT",
										BsSkin.groupLabelXOffset, BsSkin.groupLabelYOffset)
	...
	function Inventory:AssignItemsToSlots(
		...
		if groupItemCount > 0 then
			...
			-- Count item stacks if stacking is enabled.
			local stackItems = self.settings.stackItems
			if stackItems then
				self:CountItemStacks(groupId)
			end
			...
			for position = firstItem, lastItem, step do
				...
				-- Visual Item Stacking.
				-- We do this before empty slot stacking so empty slots (which are technically items)
				-- don't get caught up in this logic if they happen to share an ID (which they shouldn't).
				if stackItems and item.emptySlot == 0 and item.id then
					local uniqueId = self:GetItemStackUniqueId(item)
					...
					if stackInfo then
						if stackInfo.displayed then
							...
							if stackInfo.count > item.count then
								-- Create a proxy item to avoid modifying the actual cache.
								-- We only need to override the count.
								-- AssignItemToItemButton reads item.count directly.
								-- We'll create a lightweight table that falls back to the original item.

								local realItem = item
								item = setmetatable({
									count = stackInfo.count,
									bagshuiStackSlotCount = stackInfo.slotCount,
								}, { __index = realItem })
							end
						end
					end
	...
	--- Generate a unique ID for an item to determine if it should be stacked with another.
	---@param item table Bagshui inventory cache entry.
	---@return string uniqueId
	function Inventory:GetItemStackUniqueId(item)
		-- We need to differentiate items by more than just ID.
		-- Name, texture, quality, and itemLink (which contains enchant/suffix info) are important.
		-- We avoid using the full item string because we want to stack items that are effectively the same
		-- even if they have different uniqueIDs (the last part of the item string).
		-- But we DO want to respect suffix/enchant differences.
		-- item.itemLink is usually |c...|Hitem:ID:Enchant:Suffix:Unique|h[Name]|h|r
		-- We can try to strip the unique ID from it, or just use the other properties.

		-- Ideally, we use the item string without the uniqueId.
		-- BsItemInfo doesn't expose a "GetBaseItemString" easily, but we can construct a key.

		-- Using item.id + item.name + item.texture + item.quality + (item.suffixId or 0) + (item.enchantId or 0)
		-- Wait, item structure in Bagshui doesn't explicitly break out suffixId/enchantId in the cache always,
		-- but it does have `itemString`.
		-- itemString format: item:itemId:enchantId:suffixId:uniqueId

		local validString = item.itemString and string.len(item.itemString) > 0

		if validString then
			-- Parse the item string to get relevant parts.
			-- item:1234:0:0:0
			local _, _, itemId, enchantId, suffixId, uniqueId = string.find(item.itemString,
				"^item:(%d+):(%d+):(%d+):(%d-)")
			if itemId then
				-- We ignore uniqueId for stacking purposes.
				return string.format("%s:%s:%s", itemId, enchantId or 0, suffixId or 0)
			end
		end

		-- Fallback if parsing fails (shouldn't happen for valid items)
		return tostring(item.id) .. (item.name or "")
	end

	--- Reset the item stack cache.
	function Inventory:ResetItemStackCounts()
		self.itemStacks = self.itemStacks or {}
		for k in pairs(self.itemStacks) do
			self.itemStacks[k] = nil
		end
	end

	--- Loop through all items in a group and add their counts to the appropriate item stack.
	---@param groupId number ID of group to count item stacks for.
	function Inventory:CountItemStacks(groupId)
		self:ResetItemStackCounts()

		for _, item in pairs(self.groupItems[groupId]) do
			-- Only stack regular items, not empty slots (handled separately) or openable containers (if we want to exclude them).
			-- Also exclude items that are locked or shouldn't be moved? No, this is visual only.
			if item.emptySlot == 0 and item.id then
				local uniqueId = self:GetItemStackUniqueId(item)

				if not self.itemStacks[uniqueId] then
	...
	function Inventory:GetGroupItemCountForLayout(groupId, zeroUnlessHasItemsInBagNum)
		...
		for _, item in ipairs(self.groupItems[groupId]) do
			...
			if
				itemVisible
				and self.settings.stackItems
				and item.emptySlot == 0
				and item.id
			then
				local uniqueId = self:GetItemStackUniqueId(item)
				if seenStackIds[uniqueId] then
					itemVisible = false
				else
					seenStackIds[uniqueId] = true
				end
			end
	...
	function Inventory:ShowFrameInNextPosition(
		...
		assert(positioningTable,
			"Inventory:ShowFrameInNextPosition(): Positioning table '" ..
			tostring(positioningTableName) .. "' has not been initialized; must call InitFramePositioningTable() first!")
	...
	function Inventory:UpdateBagBar()
		...
		if showUsageSummary then
			...
			self.ui.frames.spaceSummary.bagshuiData.text:SetTextColor(self.settings.groupLabelDefault[1],
				self.settings.groupLabelDefault[2], self.settings.groupLabelDefault[3],
				self.settings.groupLabelDefault[4])
			self.ui.frames.miniSpaceSummaryBottom.bagshuiData.text:SetText(miniText)
			self.ui.frames.miniSpaceSummaryBottom.bagshuiData.text:SetTextColor(self.settings.groupLabelDefault[1],
				self.settings.groupLabelDefault[2], self.settings.groupLabelDefault[3],
				self.settings.groupLabelDefault[4])
			self.ui.frames.miniSpaceSummaryTop.bagshuiData.text:SetText(miniText)
			self.ui.frames.miniSpaceSummaryTop.bagshuiData.text:SetTextColor(self.settings.groupLabelDefault[1],
				self.settings.groupLabelDefault[2], self.settings.groupLabelDefault[3],
				self.settings.groupLabelDefault[4])
			if subText then
				self.ui.frames.spaceSummary.bagshuiData.text:ClearAllPoints()
				self.ui.frames.spaceSummary.bagshuiData.text:SetPoint("BOTTOM", self.ui.frames.spaceSummary, "CENTER", 0,
					-2)
				self.ui.frames.spaceSummary.bagshuiData.subtext:SetText(subText)
				self.ui.frames.spaceSummary.bagshuiData.subtext:SetTextColor(self.settings.groupLabelDefault[1],
					self.settings.groupLabelDefault[2], self.settings.groupLabelDefault[3],
					self.settings.groupLabelDefault[4])
				...
				self.ui.frames.miniSpaceSummaryBottom.bagshuiData.subtext:SetTextColor(
					self.settings.groupLabelDefault[1], self.settings.groupLabelDefault[2],
					self.settings.groupLabelDefault[3], self.settings.groupLabelDefault[4])
				...
				self.ui.frames.miniSpaceSummaryTop.bagshuiData.subtext:SetTextColor(self.settings.groupLabelDefault[1],
					self.settings.groupLabelDefault[2], self.settings.groupLabelDefault[3],
					self.settings.groupLabelDefault[4])
			...
			summaryWidth = math.max(self.ui.frames.spaceSummary.bagshuiData.text:GetStringWidth(),
	...
	function Inventory:UpdateToolbar()
		...
		if self.editMode then
			...
			toolbarButtons.offline.bagshuiData.tooltipText = GRAY_FONT_COLOR_CODE ..
				self.activeCharacterId .. FONT_COLOR_CODE_CLOSE
		else
		...
		if self.settings.showMoney then
			...
			for _, text in pairs(self.ui.frames.money.bagshuiData.texts) do
				text:SetTextColor(self.settings.groupLabelDefault[1], self.settings.groupLabelDefault[2],
					self.settings.groupLabelDefault[3], self.settings.groupLabelDefault[4])
			end
		else
		...
		then
			...
			-- Display cooldown.
			local cooldownStart, cooldownDuration, isOnCooldown = _G.GetContainerItemCooldown(
				self.hearthstoneItemRef.bagNum, self.hearthstoneItemRef.slotNum)
