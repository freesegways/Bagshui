...
Bagshui:LoadComponent(function()
	...
	local function ItemButton_OnEnter(targetItemButton, fromEntryFrame, refreshTooltipOnly)
		...
		if type(itemString) == "string" and string.find(itemString, "^item:") then
			...
			_G.GameTooltip.bagshuiData.showInfoTooltipWithoutAlt = this.bagshuiData and
				this.bagshuiData.showBagshuiInfoWithoutAlt
	...
	function Ui:SkinItemButton(button, buttonType)
		...
		buttonInfo.buttonComponents = {
			...
			-- Create a new font string for the stack slot count.
			slotCount = button:CreateFontString(buttonName .. "SlotCount", "ARTWORK", "NumberFontNormal"),
		}
		local buttonComponents = buttonInfo.buttonComponents

		-- Adjust texture coordinates.
		if BsSkin.itemSlotIconTexCoord then
			buttonComponents.iconTexture:SetTexCoord(BsSkin.itemSlotIconTexCoord[1], BsSkin.itemSlotIconTexCoord[2],
				BsSkin.itemSlotIconTexCoord[3], BsSkin.itemSlotIconTexCoord[4])
		end
		...
		buttonComponents.count:SetPoint("BOTTOMRIGHT", button, -BsSkin.itemSlotDecorationAnchor,
			BsSkin.itemSlotDecorationAnchor)
		buttonComponents.stock:SetPoint("TOPLEFT", button, BsSkin.itemSlotDecorationAnchor,
			-BsSkin.itemSlotDecorationAnchor)
		buttonComponents.slotCount:SetPoint("TOPRIGHT", button, -BsSkin.itemSlotDecorationAnchor,
			-BsSkin.itemSlotDecorationAnchor)
		...
		buttonComponents.slotCount:SetFont(font, fontSize, fontStyle)
		...
		buttonComponents.slotCount:SetShadowColor(0, 0, 0, 0.75)
		buttonComponents.slotCount:SetShadowOffset(1, -1)
		buttonComponents.slotCount:Hide()
		...
		buttonComponents.border:SetPoint("TOPLEFT", button, "TOPLEFT", -BsSkin.itemSlotBorderAnchor,
			BsSkin.itemSlotBorderAnchor)
		buttonComponents.border:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", BsSkin.itemSlotBorderAnchor,
			-BsSkin.itemSlotBorderAnchor)
		...
		if BsSkin.itemSlotHighlightAnchor then
			...
			buttonComponents.highlightTexture:SetPoint("TOPLEFT", -BsSkin.itemSlotHighlightAnchor,
				BsSkin.itemSlotHighlightAnchor)
			buttonComponents.highlightTexture:SetPoint("BOTTOMRIGHT", BsSkin.itemSlotHighlightAnchor,
				-BsSkin.itemSlotHighlightAnchor)
		end
		...
		if BsSkin.itemSlotPushedAnchor then
			...
			buttonComponents.pushedTexture:SetPoint("BOTTOMRIGHT", BsSkin.itemSlotPushedAnchor,
				-BsSkin.itemSlotPushedAnchor)
		end
		...
		buttonComponents.innerGlow:SetPoint("TOPLEFT", button, "TOPLEFT", -BsSkin.itemSlotInnerGlowAnchor,
			BsSkin.itemSlotInnerGlowAnchor)
		buttonComponents.innerGlow:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", BsSkin.itemSlotInnerGlowAnchor,
			-BsSkin.itemSlotInnerGlowAnchor)
		...
		if buttonType == BS_UI_ITEM_BUTTON_TYPE.ITEM then
			...
			-- Quality badge.
			buttonComponents.qualityBadge = self:CreateShadowedTexture(button,
			...
			buttonComponents.qualityBadge:SetPoint("BOTTOMLEFT", button, "BOTTOMLEFT", BsSkin.itemSlotDecorationAnchor,
				BsSkin.itemSlotDecorationAnchor)
			buttonComponents.qualityBadge:Hide()
			buttonInfo.qualityBadgeAlpha = 0

			-- Item usable badge.
			buttonComponents.topLeftBadge = self:CreateShadowedTexture(button,
				"Interface\\TargetingFrame\\UI-RaidTargetingIcons")
			...
			buttonComponents.topLeftBadge:SetPoint("TOPLEFT", button, "TOPLEFT", BsSkin.itemSlotDecorationAnchor,
				-BsSkin.itemSlotDecorationAnchor)
			...
			buttonComponents.stockBadge:SetPoint("TOPRIGHT", button, "TOPRIGHT", -BsSkin.itemSlotDecorationAnchor,
				-BsSkin.itemSlotDecorationAnchor)
	...
	function Ui:AssignItemToItemButton(button, item, groupId)
		...
		if inventory then
			...
			-- Stack slot count.
			if (item.bagshuiStackSlotCount or 0) > 1 then
				buttonComponents.slotCount:SetText(item.bagshuiStackSlotCount)
				buttonComponents.slotCount:Show()
			else
				buttonComponents.slotCount:Hide()
			end
	...
	function Ui:UpdateItemButtonColorsAndBadges(button, force)
		...
		if inventory and inventory.editMode then
			...
			if inventory then
				...
				local searchText = inventory.dockTo and Bagshui.components[inventory.dockTo].searchText or
			...
			if buttonComponents.qualityBadge then
				if buttonInfo.qualityBadgeAlpha > 0 then
					...
					self:SetShadowedTextureAlpha(buttonComponents.qualityBadge,
						(opacityOverride or buttonInfo.qualityBadgeAlpha))
				else
					buttonComponents.qualityBadge:Hide()
				end
			end
			if buttonComponents.topLeftBadge then
				if buttonInfo.topLeftBadgeAlpha > 0 then
					...
					self:SetShadowedTextureAlpha(buttonComponents.topLeftBadge,
						(opacityOverride or buttonInfo.topLeftBadgeAlpha))
				else
					buttonComponents.topLeftBadge:Hide()
				end
			end
		...
		buttonComponents.background:SetAlpha((not containerHighlight) and opacityOverride or
			(buttonInfo.backgroundTextureAlpha or 1))
	end
		...
		if button.bagshuiData.item and inventory and not inventory.editMode then
			...
			then
				...
				then
					...
					local timeElapsed = (_G.time() - item.bagshuiDate) /
						60 -- Settings are in minutes, so convert from seconds here.
